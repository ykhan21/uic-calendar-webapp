...
class DbService {
  ...
  async getAllData() {
    try {
      ...
      const response = await new Promise((resolve, reject) => {
        const query =
          "SELECT CRN,concat(`Crs Subj Cd`,' ',`Crs Nbr`) AS crs FROM course_data;";
  ...
  async searchCourse(entry) {
    try {
      ...
      const response = await new Promise((resolve, reject) => {
        // const query =
        //   searchQuery + "FROM course_data HAVING crs = ? OR crn = ?;";
        const query =
          "SELECT GROUP_CONCAT(CRN separator ',') as crn,crs,title,days,time,meetTimeDate,room FROM ( SELECT CONCAT(`Crs Subj Cd`,`Crs Nbr`) AS crs,CRN,`Course Title` AS title,`Room -- Building` AS room,`Meeting Days` AS days,`Start Time -- End Time` AS time,`Start Date -- End Date` AS meetTimeDate FROM ods_db.course_data GROUP BY crs,title,CRN,room,days,time HAVING crs=? OR CRN=? ) a GROUP BY crs,title,room,days,time,meetTimeDate;";
  ...
  async searchByCRN(crn) {
    try {
      ...
      const response = await new Promise((resolve, reject) => {
        ...
        const query = searchQuery + "FROM course_data WHERE crn = ?;";
